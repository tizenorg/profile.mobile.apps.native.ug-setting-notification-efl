...
static Evas_Object* gl_content_get_cb(void *data, Evas_Object *obj, const char *part)
{
	...
	if (data_list->item_style == ITEM_STYLE_ONE_LINE && !strcmp(part, "elm.swallow.end")) {
...
static char *_gl_option_text_get_cb(void *data, Evas_Object *obj, const char *part)
{
	...
	if (!strcmp(data, "app-notifications")) {
		...
		{
			/*TODO: localization*/
			snprintf(buf, sizeof(buf), "<font_size=30>%s</font_size>", APP_STRING("Select the apps you want to receive notifications from below."));
			return strdup(buf);
		}
